...
class VideoPlayer(QWidget):
    def __init__(self):
        ...
        self.processed_files_dir = "processedFiles"
    ...
    def convert_and_transcribe(self, input_file):
        ...
        # Prepare directory and filenames for processed files.
        base_name = os.path.splitext(os.path.basename(input_file))[0]
        processed_dir_path = self.ensure_processed_files_dir_exists(input_file)
        ...
        os.chdir(input_dir)  # Change the working directory to where the input file is located for easier file handling.
        ...
        try:
            if filetype in ("mp3", "wav"):
                ...
                subprocess.run(["whisper", "--model en", input_file])
                self.update_progress(50)  # Update progress to 50% after conversion.
                QApplication.processEvents()  

                # Generate subtitles and apply them to the converted video file.
                subFile = input_file.split('.')[0] + ".srt"
                captionedFile = input_file.split('.')[0] + "_Subtitled.mp4"
                subprocess.run(["ffmpeg", "-i", mp4file, "-vf", "subtitles=" + subFile,"-preset","ultrafast", captionedFile])
                ...
                subFile = input_file.split('.')[0] + ".srt"
                captionedFile = input_file.split('.')[0] + "_Subtitled.mp4"
                subprocess.run(["ffmpeg", "-i", input_file, "-vf", "subtitles=" + subFile,"-preset","ultrafast", captionedFile])
        ...
        # Move all generated files to the processed directory.
        shutil.move(mp4file, os.path.join(processed_dir_path, os.path.basename(mp4file)))
        shutil.move(subFile, os.path.join(processed_dir_path, os.path.basename(subFile)))
        shutil.move(captionedFile, os.path.join(processed_dir_path, os.path.basename(captionedFile)))

        # Additional files generated by the process are also moved to the processed directory.
        for extension in [".tsv", ".txt", ".vtt", ".json"]:
            new_file_path = input_file.split('.')[0] + extension
            shutil.move(new_file_path, os.path.join(processed_dir_path, os.path.basename(new_file_path)))
        ...
        if directory:
            for filename in os.listdir(directory):
                if filename.endswith((".mp3", ".mp4", ".wav", ".mov")):
                    self.transcribe_file(os.path.join(directory, filename))  
    ...
    def export_changes(self):
        ...
        if self.srt_filename:  # Ensure there's an SRT filename to work with.
            ...
            captionedFile = self.srt_filename.split('.')[0] + "_corrected.mp4"
    ...
    def on_combobox_changed(self, index):
        ...
        if index == 1:  
            ...
            self.transcribe_all_files()  # Initiate batch transcription of all suitable files in a directory.
        elif index == 4:
